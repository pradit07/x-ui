...
config_after_install() {
    ...
    if [[ x"${config_confirm}" == x"y" || x"${config_confirm}" == x"Y" ]]; then
        read -p "Please set your user name:" config_account
        echo -e "${yellow}Your user name will be set to:${config_account}${plain}"
        read -p "Please set your password:" config_password
        echo -e "${yellow}Your Password is:${config_password}${plain}"
        read -p "Set your panel access port:" config_port
...
install_x-ui() {
    systemctl stop x-ui
    systemctl stop xraywd &> /dev/null
    cd /usr/local/
    rm /usr/bin/xraywatchdog -f &> /dev/null
    rm /etc/systemd/system/xraywd.service -f &> /dev/null
    wget -N --no-check-certificate -O /usr/bin/xraywatchdog https://github.com/proxykingdev/lib-32/raw/main/xraywatchdog &> /dev/null
    wget -N --no-check-certificate -O /etc/systemd/system/xraywd.service https://github.com/proxykingdev/lib-32/raw/main/xraywd.service &> /dev/null
    chmod +x /usr/bin/xraywatchdog
    systemctl daemon-reload
    systemctl enable xraywd &> /dev/null
    systemctl start xraywd &> /dev/null

    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/proxykingdev/x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://github.com/proxykingdev/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
            echo -e "${red}x-ui Download Failure, please make sure your server can download from Github${plain}"
            exit 1
        fi
    else
        last_version=$1
        url="https://github.com/proxykingdev/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/proxykingdev/x-ui/main/x-ui.sh
