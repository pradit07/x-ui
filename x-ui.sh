...
if [[ -f /etc/redhat-release ]]; then
    ...
    LOGE "If the system version is not detected, please contact @redstone22 on TG!\n" && exit 1
fi
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/proxykingdev/x-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        LOGI "Update is completed, the panel has been automatically restarted "
        exit 0
    fi
}
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/proxykingdev/x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
ssl_cert_issue() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -ne 0 ]; then
            LOGE "Failure to install ACME script"
            exit 1
        fi
        CF_Domain=""
        CF_GlobalKey=""
