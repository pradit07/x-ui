...
func (s *Server) startTask() {
	...
	go func() {
		...
		// Statistics every 10 seconds of traffic, starting for 5 seconds for the first time, staggered with the time to restart XRAY
		s.cron.AddJob("@every 10s", job.NewXrayTrafficJob())
	}()

	// Check the situation where the traffic is exceeded and expired every 30 seconds
	s.cron.AddJob("@every 30s", job.NewCheckInboundJob())
	// Make a flow situation every day, 8:30 in Shanghai time at 8:30
